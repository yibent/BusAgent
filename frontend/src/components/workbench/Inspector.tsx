...
export function Inspector({
  ...
  const result = ["grasp", "pick_place", "place_held"].includes(
    status?.last_result?.skill ?? "",
  )
    ? status?.last_result
    : undefined;
  const routeName = result?.fallback_used
    ? "快速尝试 → 模型增强"
    : result?.route?.placement === "official_pick_place" ||
        result?.route?.grasp === "official_pick_place"
      ? "官方快速抓放"
      : "模型增强";
  const nodes = [
            ...
            <div className="inspector-section robot-summary">
              ...
              <div className="property-group">
                <Property
                  name="当前状态"
                  value={
                    manipulationPhases[status?.phase ?? ""] ??
                    status?.phase ??
                    "未连接"
                  }
                />
              ...
              {result && !status?.command_id && (
                <div className="property-group" aria-label="最近抓放结果">
                  <Property
                    name="任务结果"
                    value={manipulationStates[result.state] ?? result.state}
                  />
                  {result.route && (
                    <Property name="执行方式" value={routeName} />
                  )}
                  {result.elapsed_s != null && (
                    <Property
                      name="执行耗时"
                      value={`${result.elapsed_s.toFixed(1)} 秒`}
                    />
                  )}
                  {result.evaluation && (
                    <>
                      ...
                      <Property
                        name="物理验证"
                        value={
                          result.evaluation.physical_success ? "通过" : "未通过"
                        }
                      />
                    </>
                  )}
              ...
              {status?.holding?.verified && (
                <p className="field-help">
                  拿起后可单独说“放到蓝色托盘”或“在桌子上找个空位放下”。普通放置优先快环，随时可以停止。
                </p>
              )}
              <div className="inspector-subhead">
                <Activity size={13} />
