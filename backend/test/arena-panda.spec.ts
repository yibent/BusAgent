...
describe('Arena Panda task decisions', () => {
  it('normalizes common offline prompts without dropping unknown labels or qualifiers', () => {
    expect(visualLabel('蓝色托盘')).toBe('blue tray');
    expect(visualLabel('orange tray')).toBe('orange tray');
    expect(visualLabel('左边的蓝色托盘')).toBe('左边的蓝色托盘');
    expect(visualLabel('custom machined holder')).toBe('custom machined holder');
  });
  it.each(['放下', '把它放到蓝色托盘', '在桌子上随便找个地方放下'])(
    'plans an independent placement: %s',
    (text) => {
      ...
      semanticFrame(
        {
          intent: 'place_held',
          destination: 'table',
          placement_selection: 'free_space',
        },
        '在桌子上随便找个地方放下',
      ),
      'free',
    )!;
    expect(plan.steps[0]?.params.destination).toEqual({
      type: 'named_region',
      label: 'table',
  ...
  it('uses measured holding to resume a named object without another grasp', () => {
    const parsed = semanticFrame(
      { intent: 'pick_place', category: 'gear', destination: 'blue tray' },
      '把齿轮放到蓝色托盘',
    );
    const held = { holding: { verified: true, label: 'gear' } };
    expect(buildPlan(routeHeldPlacement(parsed, held), 'resume')?.steps[0]?.skill).toBe(
      'place_held',
    );
    expect(
      routeHeldPlacement(parsed, { holding: { verified: false, label: 'gear' } })
        .intent,
    ).toBe('pick_place');
    expect(
      routeHeldPlacement(parsed, { holding: { verified: true, label: 'bolt' } }).intent,
    ).toBe('pick_place');
    const other = semanticFrame(
      {
        intent: 'pick_place',
        category: 'block',
        color: 'red',
    ...
    expect(
      routeHeldPlacement(other, { holding: { verified: true, label: 'green block' } })
        .intent,
    ).toBe('pick_place');
  });
  ...
  it('removes the support-relation suffix from an offline placement label', () => {
    ...
    try {
      expect(
        parseInstruction('把红色方块放到黄色圆柱的上面').destination,
      ).toMatchObject({ label: 'yellow cylinder' });
    } finally {
      vi.unstubAllEnvs();
    }
  });
